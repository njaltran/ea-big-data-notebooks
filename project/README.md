...
# configure dlt secrets (if needed)
# edit .dlt/secrets.toml with API keys
...
All pipeline commands run from `project/`, not the repo root, because dlt resolves `.dlt/secrets.toml` and configs from cwd.
